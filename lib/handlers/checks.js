                ...
                _data.read('users', tokenData.phone, (err, userData) => {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            ...
                            //Cerify the given url has DNS entries (and therefore can resolve)
...
module.exports.get = (data, callBack) => {
    ...
    if (id) {
        _performance.mark('reading check')
        //Lookup the check
        _data.read('checks', id, (err, checkData) => {
            if (!err && checkData) {
                _performance.mark('verifying token')
                //Get the token from the headers
                const token = typeof (data.headers.token) == 'string' ? data.headers.token : false
                ...
                _performance.mark('check got it')

                //Gather all the measurements
                _performance.measure('Whole process', 'entered function', 'check got it')
                _performance.measure('Reading check', 'reading check', 'verifying token')
